<template>
  <div ref="main" class="edit__main" @click="cancaleSelect($event)">
    <div ref="wrap" class="edit">
      <read-slide v-if="showRead" ref="readSlide" @close="close" />

      <div ref="ppt" class="edit__wrap" />

      <div class="ppt_page">第&nbsp;{{ currentPage + 1 }}&nbsp;页</div>
      <div class="edit__operation">
        <div v-show="!showTitle" class="edit__select">

          <div class="edit__box edit__title" @click="toBack">
            <div class="edit__icon back_icon" />
            <div class="title_tip">上一页</div>
          </div>

          <div class="edit__box">
            <label class="edit__icon bac_icon">
              <input ref="bacSource" type="file" style=" display: none;" @change="addImg($event, 'bacImg')">
            </label>
            <div class="title_tip">背景图</div>
          </div>

          <div class="edit__box edit__title" @click="addTitle">
            <div class="edit__icon title_icon" />
            <div class="title_tip">文本框</div>
          </div>

          <div class="edit__box edit__title" @click="save">
            <div class="edit__icon sava_icon" />
            <div class="sava_tip">保存</div>
          </div>

          <div class="edit__box edit__title" @click="openRead">
            <div class="edit__icon read_icon" />
            <div class="title_tip">预览</div>
          </div>

          <div class="edit__box edit__title" @click="addVideo">
            <div class="edit__icon video_icon" />
            <div class="title_tip">视频</div>
          </div>

          <div class="edit__box edit__title">
            <div class="edit__icon box__icon" @click="openBacWrap" />
            <div class="title_tip">背景色</div>
            <transition name="fade">
              <div v-show="showBacWrap" class="bac__wrap">
                <div class="bac__box"><input v-model="bacLeft" type="text" class="font__input bac__input"> <span class="bac__color" :style="{backgroundColor: bacLeft, boxShadow: bacShadowLeft}" @click="openBacColor(true)" /></div>
                <div class="bac__box"><input v-model="bacRight" type="text" class="font__input bac__input"> <span class="bac__color" :style="{backgroundColor: bacRight, boxShadow: bacShadowRight}" @click="openBacColor(false)" /></div>

                <transition name="fade">
                  <div v-show="showBacColor" class="color__picker color__picker--bac">
                    <div ref="bacColor" class="cp-default cx-default" />
                  </div>
                </transition>
              </div>
            </transition>
          ...
          <div class="edit__box edit__title">
            <label class="edit__icon img_icon">
              <input ref="source" type="file" style=" display: none;" @change="addImg($event, 'img')">
            </label>
            <div class="title_tip">图片</div>
          </div>

          <div class="edit__box edit__title" @click="toNext">
            <div class="edit__icon next_icon" />
        ...
        <!-- 文字操作框 -->
        <div v-show="showTitle" class="title__operation">

          <div class="edit__box" @click="cancelDom">
            <div class="edit__icon cancel_icon" />
            <div class="title_tip">删除</div>
          </div>

          <div v-show="domType==='title'" class="edit__box" @click="addBold">
            <div class="edit__icon B_icon" :class="{BB_icon: currentBold==='bold'}" />
            <div class="title_tip">加粗</div>
          </div>

          <div class="edit__box" @click="showMove= !showMove">
            <div class="edit__icon move_icon" />
            <div class="title_tip">动画</div>

            <transition name="bounce">
              <div v-show="showMove" class="move__select">
                <div
                  v-for="(item, index) in moveList"
                  :key="index"
                  class="move__btn"
                  :class="{slect__move: index==currentMove }"
                  @click.stop="addMove(item, index)"
                >
                  <span class="animated infinite" style="animation-duration: 2s;" :class="item">Animate</span></div>
              </div>
            </transition>
          </div>
          ...
          <template v-if="domType=='video'">
            <div class="edit__box">
              <div class="title__size"><span class="title__name">地址:</span><input v-model="videoSrc" class="font__input" style="border-bottom: 1px solid white;" type="text"></div>
            </div>

            <div class="edit__box">
              <div class="title__size"><span class="title__name">模糊度</span><input v-model="blur" class="font__input" type="text"></div>
            </div>
          </template>

          <div class="edit__box">
            <div v-if="domType==='title'" class="title__size"><span class="title__name">size:</span><input v-model="fontSize" class="font__input" type="text"></div>
            <div v-if="domType==='img'" class="title__size"><span class="title__name">opacity:</span><input v-model="currentOpacity" class="font__input" type="text"></div>
            <div class="title__size"><span class="title__name">level:</span><input v-model="currentLevel" class="font__input" type="text"></div>
          </div>

          <div class="edit__box">
            <div class="title__size"><span class="title__name">宽度:</span><input v-model="currentWidth" class="font__input" type="text" @key="down($event)"></div>
            <div class="title__size"><span class="title__name">高度:</span><input v-model="currentHeight" class="font__input" type="text"></div>
          </div>

          <div class="edit__box">
            <div class="title__size"><span class="title__name" style="width:100px;">出场顺序:</span><input v-model="currentOrder" class="font__input" type="text"></div>
            <div class="title__size"><span class="title__name" style="width:100px">离场顺序:</span><input v-model="currentLeaveOrder" class="font__input" type="text"></div>
          </div>

          <div class="edit__box">
            <div class="title__size"><span class="title__name">X坐标:</span><input v-model="currentX" class="font__input" type="text"></div>
            <div class="title__size"><span class="title__name">Y坐标:</span><input v-model="currentY" class="font__input" type="text"></div>
          </div>

          <div class="edit__box">
            <div class="title__size"><span class="title__name">颜色:</span><span ref="color1" class="color__box" @click="openColor(0, $event)" /></div>
            <div class="title__size"><span class="title__name">背景:</span><span ref="color2" class="color__box" @click="openColor(1, $event)" /></div>
            <transition name="fade">
              <div v-show="showColor" class="color__picker">
                <div ref="color" class="cp-default cx-default" />
              </div>
            </transition>
          </div>
...
export default {
  ...
  watch: {
    ...
    bacLeft(newValue) {
      const str = `background-image:linear-gradient(135deg, ${newValue} 0%, ${this.bacRight} 100%);`
      this.$refs.ppt.style = str
      this.bacColorList[this.currentPage] = str
    },

    bacRight(newValue) {
      const str = `background-image:linear-gradient(135deg, ${this.bacLeft} 0%, ${newValue} 100%);`
      this.$refs.ppt.style = str
      this.bacColorList[this.currentPage] = str
    }
  ...
  methods: {
    ...
    openBacColor(who) {
      const str = '0px 0px 5px 4px #9face6'
      if (who) {
    ...
    // 保存数据
    save() {
      const list = Array.from(this.$refs.ppt.children)
      const temp = []
      list.forEach((item) => {
    ...
    addBold() {
      const temp = this.currentDom.style.fontWeight || ''
      if (temp === 'bold') {
